...
import { Question, Player, AnswerResult, LeaderboardEntry } from '../types';
...
export const answerQuestion = async (userId: string, questionId: string, answer: string): Promise<AnswerResult> => {
  const { data } = await api.post('/quiz/answer', { userId, questionId, answer });
  return data;
};
